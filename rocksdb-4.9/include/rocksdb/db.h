...
// A range of keys, Shichao
struct Range {
  Slice start;          // Included in the range
  Slice limit;          // Not included in the range
  SequenceNumber seq;   // <= Seq for range query

  static const SequenceNumber kMaxSequenceNumber = ((0x1ull << 56) - 1);

  Range() : seq(kMaxSequenceNumber) { }
  Range(const Slice& s, const Slice& l,
        SequenceNumber sn = kMaxSequenceNumber) : start(s), limit(l), seq(sn) { }
};
...
class DB {
 public:
  ...
  virtual void WaitBackgroundWork() {}
...
#undef DeleteFile
  ...
  virtual bool NoBackgroundWork() { return false; }  // Shichao
