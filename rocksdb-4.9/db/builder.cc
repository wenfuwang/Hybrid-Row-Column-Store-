...
Status BuildTable(
  ...
  if (iter->Valid()) {
    ...
    CompactionIterator c_iter(iter, internal_comparator.user_comparator(),
                              ...
                              true /* internal key corruption is not ok */);
    c_iter.SeekToFirst();
    steady_clock::time_point t5;
    steady_clock::time_point t6;
    steady_clock::time_point t7;
    steady_clock::time_point t8;
    double datatime = 0, indextime = 0;
    for (; c_iter.Valid(); c_iter.Next()) {
      ...
      t6 = steady_clock::now();
      datatime += duration_cast<duration<double>>(t6 - t5).count();
    ...
    if (!s.ok() || empty) {
      ...
      s = builder->Finish();
      t8 = steady_clock::now();
      indextime += duration_cast<duration<double>>(t8 - t7).count();
    }

    std::cout<<datatime<<", "<<indextime<<std::endl;
