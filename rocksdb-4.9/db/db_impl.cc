...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname)
    : env_(options.env),
      ...
      unscheduled_trans_(0),
      unscheduled_compactions_(0),
      bg_compaction_scheduled_(0),
      num_running_compactions_(0),
      bg_trans_scheduled_(0),
...
DBImpl::~DBImpl() {
  ...
  int trans_unscheduled = env_->UnSchedule(this, Env::Priority::MEDIUM);
  int flushes_unscheduled = env_->UnSchedule(this, Env::Priority::HIGH);
  mutex_.Lock();
  bg_compaction_scheduled_ -= compactions_unscheduled;
  bg_trans_scheduled_ -= trans_unscheduled;
  bg_flush_scheduled_ -= flushes_unscheduled;
  // Wait for background work to finish
  while (bg_compaction_scheduled_ || bg_trans_scheduled_ || bg_flush_scheduled_) {
    bg_cv_.Wait();
  }
...
void DBImpl::MaybeScheduleTrans() {
  mutex_.AssertHeld();
  if (shutting_down_.load(std::memory_order_acquire)) {
    // DB is being deleted; no more background compactions
    return;
  ...
  while (unscheduled_trans_ > 0 && bg_trans_scheduled_ < 1) {
    unscheduled_trans_--;
    bg_trans_scheduled_++;
    env_->Schedule(&DBImpl::BGWorkTrans, this, Env::Priority::MEDIUM, this);
  }
}
...
void DBImpl::BGWorkTrans(void* db) {
  reinterpret_cast<DBImpl*>(db)->BackgroundCallTrans();
}
...
void DBImpl::BackgroundCallTrans() {
  InstrumentedMutexLock l(&mutex_);

  JobContext job_context(next_job_id_.fetch_add(1), true);
  Status s = BackgroundTrans(&job_context);
...
Status DBImpl::BackgroundTrans(JobContext* job_context) {
  mutex_.AssertHeld();
  ...
  std::unordered_map<FileMetaData*, FileMetaData> shadow;
  ProcessTrans(cfd, v, shadow);

  VersionEdit edit;
  edit.SetColumnFamily(cfd->GetID());
  edit.AddFileShadow(shadow);
  ...
  t2 = steady_clock::now();
  cout << "compact seconds: " << duration_cast<duration<double>>(t2 - t1).count() << endl;

  return status;
}

void DBImpl::ProcessTrans(ColumnFamilyData* cfd, Version* v,
                          std::unordered_map<FileMetaData*, FileMetaData>& shadow) {
  VersionStorageInfo* storage = v->storage_info();
  for (int i=0; i<storage->num_non_empty_levels(); ++i) {
//    if (i != 1) continue;
    const rocksdb::LevelFilesBrief& levelFilesBrief = storage->LevelFilesBrief(i);
    for (size_t j=0; j<levelFilesBrief.num_files; ++j) {
      cout<<levelFilesBrief.files[j].fd.GetNumber()<<","<<levelFilesBrief.files[j].fd.file_type<<",";

      InternalIterator* iter = cfd->table_cache()->NewIterator(
          ReadOptions(), env_options_, cfd->internal_comparator(),
          levelFilesBrief.files[j].fd, nullptr, nullptr, true);

      Status s;
      FileMetaData meta;
      meta.fd = FileDescriptor(versions_->NewFileNumber(), 0, 0);

      std::string fname = TableFileName(cfd->ioptions()->db_paths, meta.fd.GetNumber(),
                                        meta.fd.GetPathId());
      ...
      {
        unique_ptr<WritableFile> file;
        s = NewWritableFile(env_, fname, &file, env_options_);

        file_writer.reset(new WritableFileWriter(std::move(file), env_options_));

        if (levelFilesBrief.files[j].fd.file_type == "BlockBasedTable") {
            shared_ptr<TableFactory> column_table_factory(NewColumnTableFactory());
            static_cast<ColumnTableOptions*>(column_table_factory->GetOptions())->column_num = 14;

            builder = column_table_factory->NewTableBuilder(
                TableBuilderOptions(*cfd->ioptions(), cfd->internal_comparator(), nullptr,
                    GetCompressionType(*cfd->ioptions(), storage,
                        *cfd->GetLatestMutableCFOptions(), i, storage->base_level()),
                    cfd->ioptions()->compression_opts, nullptr, false, cfd->GetName(), env_options_),
                    cfd->GetID(), file_writer.get());
        }

        if (levelFilesBrief.files[j].fd.file_type == "ColumnTable") {
          shared_ptr<TableFactory> block_based_factory(NewBlockBasedTableFactory());
          builder = block_based_factory->NewTableBuilder(
              TableBuilderOptions(*cfd->ioptions(), cfd->internal_comparator(), nullptr,
                  GetCompressionType(*cfd->ioptions(), storage,
                      *cfd->GetLatestMutableCFOptions(), i, storage->base_level()),
                  cfd->ioptions()->compression_opts, nullptr, false, cfd->GetName(), env_options_),
      ...
      for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
        ...
        meta.UpdateBoundaries(key, ikey.sequence);
      }

      cout<<":"<<meta.fd.GetNumber()<<",";
      ...
      if (s.ok() && !empty) {
          ...
          meta.fd.file_type = builder->Name();
          meta.fd.file_size_total = builder->FileSizeTotal();
          meta.marked_for_compaction = false;
      }
      delete builder;

      if (s.ok() && !empty && !cfd->ioptions()->disable_data_sync) {
        file_writer->Sync(cfd->ioptions()->use_fsync);
      }
      if (s.ok() && !empty) {
        s = file_writer->Close();
        meta.compensated_file_size = i;
        shadow[storage->LevelFiles(i)[j]] = meta;
      }
    }
    cout<<endl;
  }
