...
struct FileMetaData {
  ...
  Cache::Handle* table_reader_handle_shadow;
  ...
  FileMetaData()
      : refs(0),
        ...
        table_reader_handle_shadow(nullptr),
...
class VersionEdit {
 public:
  ...
  void AddFileShadow(std::unordered_map<FileMetaData*, FileMetaData>& shadow) {
    for (auto& it : shadow) {
      int level = it.second.compensated_file_size;
      deleted_files_.insert({level, it.first->fd.GetNumber()});
      new_files_.emplace_back(level, *(it.first));
      shadow_files_[it.first->fd.GetNumber()] = it.second;

      new_files_.back().second.fd_shadow = it.second.fd;
      new_files_.back().second.table_reader_handle_shadow = it.second.table_reader_handle;
    }
  }
  ...
  std::unordered_map<uint64_t, FileMetaData> GetShadowFiles() {
    return shadow_files_;
  }
 ...
 private:
  ...
  std::unordered_map<uint64_t, FileMetaData> shadow_files_;
