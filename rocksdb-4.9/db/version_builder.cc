...
class VersionBuilder::Rep {
 ...
 public:
  ...
  void UnrefFile(FileMetaData* f) {
    ...
    if (f->refs <= 0) {
      ...
      if (f->table_reader_handle_shadow) {
        assert(table_cache_ != nullptr);
        table_cache_->ReleaseHandle(f->table_reader_handle_shadow);
  ...
  void Apply(VersionEdit* edit) {
    ...
    shadow_files_ = edit->GetShadowFiles();
    ...
    for (int level = 0; level < base_vstorage_->num_levels(); level++) {
      ...
      for (const auto& added : added_files) {
        ...
        for (auto bpos = std::upper_bound(base_iter, base_end, added, cmp);
          ...
          if (shadow_files_.empty() ||
              shadow_files_.find((*base_iter)->fd.GetNumber()) == shadow_files_.end()) {
...
//        assert(!file_meta->table_reader_handle);
    ...
    std::function<void()> load_handlers_func = [&]() {
      while (true) {
        ...
        if (file_meta->fd_shadow.GetNumber() != 0) {
          table_cache_->FindTable(
              env_options_, *(base_vstorage_->InternalComparator()),
              file_meta->fd_shadow, &file_meta->table_reader_handle_shadow, false /*no_io */,
              true /* record_read_stats */,
              internal_stats->GetFileReadHist(level), false, level);
          if (file_meta->table_reader_handle_shadow != nullptr) {
             // Load table_reader
             file_meta->fd_shadow.table_reader = table_cache_->GetTableReaderFromHandle(
                 file_meta->table_reader_handle_shadow);
          }
        }
      }
    };
