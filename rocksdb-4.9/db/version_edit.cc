...
enum Tag {
  ...
  kNewFile5 = 105,      // 5th (the latest) format version of adding files
...
bool VersionEdit::EncodeTo(std::string* dst) const {
  ...
  for (size_t i = 0; i < new_files_.size(); i++) {
    ...
    if (file5) {
      PutVarint64(dst, f.fd_shadow.GetNumber());
      PutVarint64(dst, f.fd_shadow.GetFileSize());
    }
...
Status VersionEdit::DecodeFrom(const Slice& src) {
  ...
  while (msg == nullptr && GetVarint32(&input, &tag)) {
    switch (tag) {
      ...
      case kNewFile5: {
        uint64_t number, number2, file_size2;
        uint64_t file_size;
        Slice file_type;
        uint64_t file_size_total;
        if (GetLevel(&input, &level, &msg) && GetVarint64(&input, &number) &&
            GetVarint64(&input, &file_size) &&
            GetLengthPrefixedSlice(&input, &file_type) &&
            GetVarint64(&input, &file_size_total) &&
            GetInternalKey(&input, &f.smallest) &&
            GetInternalKey(&input, &f.largest) &&
            GetVarint64(&input, &f.smallest_seqno) &&
            GetVarint64(&input, &f.largest_seqno) &&
            GetVarint64(&input, &number2) && GetVarint64(&input, &file_size2)) {
          f.fd = FileDescriptor(number, 0, file_size,
              file_type.ToString(), file_size_total);
          f.fd_shadow = FileDescriptor(number2, 0, file_size2,
              file_type.ToString()=="BlockBasedTable"? "ColumnTable": "BlockBasedTable", file_size_total);
          new_files_.push_back(std::make_pair(level, f));
        } else {
          if (!msg) {
            msg = "new-file5 entry";
