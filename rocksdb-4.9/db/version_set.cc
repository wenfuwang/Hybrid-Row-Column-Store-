...
class FilePicker {
 public:
  FilePicker(std::vector<FileMetaData*>* files, const Slice& user_key,
      ...
      : num_levels_(num_levels),
        ...
        returned_curr_index_(static_cast<unsigned int>(-1)),  // Shichao
  ...
  int GetCurrentIndex() { return returned_curr_index_; }  // Shichao
 ...
 private:
  ...
  unsigned int returned_curr_index_;  // Shichao
  ...
  for (int level = 0; level < storage_info_.num_levels_; level++) {
    for (size_t i = 0; i < storage_info_.files_[level].size(); i++) {
      ...
      if (f->refs <= 0) {
        ...
        if (f->table_reader_handle_shadow) {
          cfd_->table_cache()->EraseHandle(f->fd_shadow, f->table_reader_handle_shadow);
          f->table_reader_handle_shadow = nullptr;
        }
    ...
    *status = table_cache_->Get(
    ...
    switch (get_context.State()) {
      ...
      case GetContext::kFound: {
        ...
        Query query(0, fp.GetHitFileLevel());
        advisor.Push(query);
        return;
      }
...
//    std::cout << f->fd.GetNumber() << std::endl;
    if (range.SequenceNum() >=
        storage_info_.files_[fp.GetCurrentLevel()][fp.GetCurrentIndex()]->smallest_seqno) {
      FileDescriptor& fd_shadow = storage_info_.files_[level][index]->fd_shadow;
...
void Version::AddLiveFiles(std::vector<FileDescriptor>* live) {
  for (int level = 0; level < storage_info_.num_levels(); level++) {
    ...
    for (const auto& file : files) {
      ...
      if (file->fd_shadow.GetNumber() != 0) live->push_back(file->fd_shadow);
    }
  }
}
...

    {
      ...
      for (int level = 0; level < cfd->NumberLevels(); level++) {
        for (const auto& f :
          ...
          if (f->fd_shadow.GetNumber() != 0) {
            edit.new_files_.back().second.fd_shadow = f->fd_shadow;
          }
        }
      }
