...
class DBImpl : public DB {
 public:
  ...
  virtual void WaitBackgroundWork() override {
    mutex_.Lock();
    while (unscheduled_compactions_ || bg_compaction_scheduled_) bg_cv_.Wait();
    mutex_.Unlock();
  }
  ...
  // Shichao
  bool NoBackgroundWork() { return flush_queue_.empty() && compaction_queue_.empty(); }
 ...
 private:
  ...
  void MaybeScheduleTrans();
  void SchedulePendingFlush(ColumnFamilyData* cfd);
  void SchedulePendingCompaction(ColumnFamilyData* cfd);
  static void BGWorkCompaction(void* arg);
  static void BGWorkTrans(void* db);
  static void BGWorkFlush(void* db);
  static void UnscheduleCallback(void* arg);
  void BackgroundCallCompaction(void* arg);
  void BackgroundCallTrans();
  void BackgroundCallFlush();
  Status BackgroundCompaction(bool* madeProgress, JobContext* job_context,
                              LogBuffer* log_buffer, void* m = 0);
  Status BackgroundTrans(JobContext* job_context);
  void ProcessTrans(ColumnFamilyData* cfd, Version* v,
